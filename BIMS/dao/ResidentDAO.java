    ...
    public class ResidentDAO {
        ...
        public void reloadFromDisk() {
            System.out.println("Reloading data from disk...");
            File file = new File(DATA_FILE);
            if (file.exists()) {
                try (ObjectInputStream ois = new ObjectInputStream(new FileInputStream(file))) {
                    @SuppressWarnings("unchecked")
                    List<Resident> loadedResidents = (List<Resident>) ois.readObject();
                    this.residents = loadedResidents;
                    System.out.println("Reloading complete. Total residents: " + residents.size());
                } catch (Exception e) {
                    System.err.println("Error reloading residents from disk: " + e.getMessage());
                    // Optionally, clear or reset to empty list if needed
                    this.residents = new ArrayList<>();
                }
            } else {
                // If file doesn't exist, ensure list is empty
                this.residents = new ArrayList<>();
            }
        }
    }
